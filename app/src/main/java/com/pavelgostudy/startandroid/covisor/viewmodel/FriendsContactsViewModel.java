...
public class FriendsContactsViewModel extends ViewModel {
    private MutableLiveData<ContactsGroupedRs> groupedByFriendsLiveData = new MutableLiveData<>();
    private GroupedContactsRepository contactsRepository = MyApp.getInstance().groupedContactsRepository;

    public void getContactsGroupedByFriends(String empid) {
        contactsRepository.getContactsGroupedByOrder(3, empid,
                new GroupedContactsRepository.GetGroupedContactsCallBack() {
                    @Override
                    public void onSuccess(ContactsGroupedRs response) {
                        groupedByFriendsLiveData.setValue(response);
                    }
    ...
    public MutableLiveData<ContactsGroupedRs> getGroupedByFriendsLiveData() {
        return groupedByFriendsLiveData;
    }
}