...
public class OtherContactsViewModel extends ViewModel {
    private MutableLiveData<ContactsGroupedRs> groupedByOtherLiveData = new MutableLiveData<>();
    private GroupedContactsRepository contactsRepository = MyApp.getInstance().groupedContactsRepository;

    public void getContactsGroupedByOther(String empid) {
        contactsRepository.getContactsGroupedByOrder(4, empid,
                new GroupedContactsRepository.GetGroupedContactsCallBack() {
                    @Override
                    public void onSuccess(ContactsGroupedRs response) {
                        groupedByOtherLiveData.setValue(response);
                    }
    ...
    public MutableLiveData<ContactsGroupedRs> getGroupedByOtherLiveData() {
        return groupedByOtherLiveData;
    }
}